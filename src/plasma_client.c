...
int plasma_subscribe(plasma_connection *conn) {
   ...
   */
int connect_retry(const char *socket_name, int num_retries, int64_t timeout) {
  CHECK(socket_name);
  int fd = -1;
...
plasma_connection *plasma_connect(const char *store_socket_name,
  ...
  result->store_conn = connect_retry(store_socket_name, 50, 100);
  if (manager_socket_name != NULL) {
    result->manager_conn = connect_retry(manager_socket_name, 50, 100);
  } else {
